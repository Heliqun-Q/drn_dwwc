...
import netron    
import torch.onnx 
drn_dwwc = DRN_DWWC(num_classes=10) 
...
viz = Visdom()
viz.line([[0.0, 0.0]], [0.], win='loss', opts=dict(title='loss', legend=['train_loss', 'test_loss']))
global_step = 0
...
for epoch in range(epochs):
    for step,(x,y) in enumerate(train_loader):
        ...
        global_step += 1
        test_out = drn_dwwc(x_valid)
        test_loss = loss_func(test_out, y_valid.long())
        # pred_y = torch.max(test_out,1)[1].data.squeeze()
        # accuracy = sum(pred_y == test_y)/ test_y.size
        viz.line([[loss.item(), test_loss.item()]], [global_step], win='loss', update='append')
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.out = nn.Linear(32*16*16,9)#输出为6类
...
#opimizer = optim.Adam(cnn.parameters(), lr=lr, weight_decay=5.0)
opimizer = optim.Adam(cnn.parameters(), lr=lr, weight_decay=0.001)
loss_func = nn.CrossEntropyLoss()
